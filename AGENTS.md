...
- 当开发内容涉及海康 SDK、ISAPI、超脑、网络摄像头、Web 控件、预览、抓图、布防、远程配置等能力时，必须优先查阅 `设备网络SDK编程指南（明眸-以人为中心）/` 下的本地手册、索引和对照文档，再决定实现方案。
- 若本地手册没有覆盖对应接口、结构体、错误码或设备能力，必须继续查官方文档或联网搜索核实，优先使用海康官方资料；不要在文档未确认的情况下凭经验猜接口行为、参数含义、返回值或设备限制。
- 对拿不准的 SDK 细节，宁可先查文档、查源码调用点、查官方页面，也不要“看起来差不多就直接写”。如果最终仍查不到，需在回答或代码说明里明确标注不确定点和依据。
